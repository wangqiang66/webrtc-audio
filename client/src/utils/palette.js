...
class Palette {
  ...
  onmousemove(e) { // 鼠标移动
    ...
    const now = [endx, endy] // 当前移动到的位置
    switch (this.drawType) {
      case 'line' : {
        const params = [this.last, now, this.lineWidth, this.drawColor]
      ...
      case 'rect' : {
        ...
        break
      case 'polygon' : {
        const params = [this.x, this.y, this.sides, width, height, this.lineWidth, this.drawColor]
      ...
      case 'arc' : {
        ...
        break
      case 'eraser' : {
        const params = [endx, endy, this.width, this.height, this.lineWidth]
  ...
  polygon(x, y, sides, width, height, lineWidth, drawColor) { // 绘制多边形
    ...
    const n = sides
    const ran = 360 / n
    const rn = Math.sqrt(Math.pow(width, 2) + Math.pow(height, 2))
    this.paint.beginPath()
    this.paint.strokeStyle = drawColor
    this.paint.lineWidth = lineWidth
    for (let i = 0; i < n; i++) {
